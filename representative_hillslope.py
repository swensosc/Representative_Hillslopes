...
def CalcRepresentativeHillslopeForm(hillslope_fraction, \
    ...
    if form == 'CircularSection':
        ...
        return {'n_valid_bins':nvbins,'node_distance':ndmed,'edge_width':nwidth,'area':narea,'hill_length':mean_hill_radius}
    ...
    if form == 'TriangularSection':
        ...
        return {'n_valid_bins':nvbins,'node_distance':ndmed,'edge_width':nwidth,'area':narea,'hill_length':mean_hill_length}

def CalcGeoparamsGridcell(ji, \
                      lon2d=None, \
                      lat2d=None, \
                      landmask=None, \
                      nhand_bins=None, \
    ...
    if landmask[j,i] == 1:
        ...
        gsf = 0.5
        scorners = [[lon2d[j,i]-gsf*dlon,lat2d[j,i]-gsf*dlat],
        ...
        if verbose:
            print('spatial scale in meters, grid spacing ',scale_in_meters,grid_spacing)
        ...
        hcorners = [[lon2d[j,i]-0.5*dlonh,lat2d[j,i]-0.5*dlath],
        ...
        if fhand.size > 0:
            hand_coverage_fraction = np.sum(np.where(fhand[np.isfinite(fhand)] > 0,1,0))/fhand.size
        else:
        ...
        if num_flooded_pts > 0:
            ...
            for ft in np.linspace(0,20,50):
                if (np.sum((np.abs(fflood[fhand < hand_threshold]) > ft))/num_flooded_pts) < 0.95:
                    flood_thresh = ft
                    break

            fhand = np.where(np.logical_and(np.abs(fflood) > flood_thresh,fhand < hand_threshold),-1,fhand)
        ...
        if verbose:
            ...
            print('mean catchment area ',np.mean(carea),np.sum(farea[np.isfinite(fhand)])/stream_number)
        ...
        hand_bin_bounds = SpecifyHandBounds(fhand,faspect,aspect_bins,bin1_max=2,BinMethod='fastsort')
        ...
        for asp_ndx in range(naspect):
            ...
            if asp_ndx == 0:
                aind = np.where(np.logical_or(faspect >= aspect_bins[asp_ndx][0],faspect < aspect_bins[asp_ndx][1]))[0]
            else:
                aind = np.where(np.logical_and(faspect >= aspect_bins[asp_ndx][0],faspect < aspect_bins[asp_ndx][1]))[0]
            ...
            if aind.size > 0:
                ...
                for n in range(nhand_bins):
                    ...
                    if cind.size > 0:
                        ...
                        dtnd[asp_ndx*nhand_bins+n] = dtnd_sorted[int(0.5*dtnd_sorted.size-1)]
                        ...
                        mean_aspect = np.arctan2(np.mean(np.sin(dtr*faspect[cind])),np.mean(np.cos(dtr*faspect[cind]))) / dtr
                        if mean_aspect < 0:
                            mean_aspect += 360.
                        ...
                        if n == 0:
                            downhill_column_index[asp_ndx*nhand_bins+n] = lowland_index
                        else:
                            downhill_column_index[asp_ndx*nhand_bins+n] = col_cnt -1
                        col_cnt += 1
                            
                        if verbose:
                            print('chk h/d/a: ',n,hand[asp_ndx*nhand_bins+n],dtnd[asp_ndx*nhand_bins+n],area[asp_ndx*nhand_bins+n])

                # identify lowland column with first nonzero column index
                for n in range(nhand_bins):
                    if column_index[asp_ndx*nhand_bins+n] > 0:
                        if downhill_column_index[asp_ndx*nhand_bins+n] > lowland_index:
                            downhill_column_index[asp_ndx*nhand_bins+n] = lowland_index
                        break
            ...
            if printData:
                ...
                print('height ',hand[asp_ndx*nhand_bins:asp_ndx*nhand_bins+nhand_bins])
                print('width ',width[asp_ndx*nhand_bins:asp_ndx*nhand_bins+nhand_bins])
                print('distance ',dtnd[asp_ndx*nhand_bins:asp_ndx*nhand_bins+nhand_bins])
                print('area ',area[asp_ndx*nhand_bins:asp_ndx*nhand_bins+nhand_bins])
                print('colndx ',column_index[asp_ndx*nhand_bins:asp_ndx*nhand_bins+nhand_bins])
        ...
        if not printData:
            ...
            olon2d  = w.createVariable('LONGXY',float,('lsmlat','lsmlon',))
            olon2d.units = 'degrees'
            olon2d.long_name = 'longitude - 2d'

            olat2d  = w.createVariable('LATIXY',float,('lsmlat','lsmlon',))
            ...
            ocmask  = w.createVariable('chunk_mask',np.int32,('lsmlat','lsmlon',))
...
class LandscapeCharacteristics(object):
    ...
    def CalcLandscapeCharacteristicsPysheds(self,corners,accum_thresh=0, \
                                            dem_file_template=None, \
                                            fill_value = -9999, \
        ...
        if type(pshape) != type(None):
            value = 1
            pmask = rasterio.features.rasterize([(pshape,value)], out_shape=(elev.shape), fill=0, out=None, transform=eaffine)
        ...
        grid = Grid.from_array(data=elev, data_name='dem', affine=eaffine,
                                     shape=elev.shape, crs=ecrs,
                                     nodata=fill_value, metadata={})
        ...
        try:
            grid.resolve_flats('flooded_dem', out_name='inflated_dem',nodata_in=fill_value)
            if verbose:
        ...
        if maskFlooded:
            if num_flooded_pts > 0:
                ...
                for ft in np.linspace(0,20,50):
                    frac_below_ft = np.sum((np.abs(fflood[fflood > 0]) < ft))/num_flooded_pts
        ...
        grid.flowdir(data='inflated_dem', out_name='dir', dirmap=dirmap,nodata_in=fill_value)
        if verbose:
        ...
        acc_mask = np.logical_and((acc_raster>self.thresh),(grid.inflated_dem != fill_value))
        
        try:
            branches = grid.extract_river_network(fdir=dir_raster, mask=acc_mask, dirmap=dirmap)
        except MemoryError:
            print('Memory Error in extract_river_network, skipping')
            return -1

        network=branches['features'] #list of features (<class 'geojson.feature.Feature'>)
        if verbose:
        ...
        try:
            x = grid.river_network_length_and_slope(fdir=dir_raster, mask=acc_mask, dirmap=dirmap)
        except MemoryError:
        ...
        if useConsistentChannelMask:
            ...
            grid.add_gridded_data(np.asarray(dmask),'channel_mask',affine=grid.affine,crs=grid.crs)
            grid.add_gridded_data(np.asarray(did),'channel_id',affine=grid.affine,crs=grid.crs)
        ...
        grid.compute_hand(fdir='dir',dem='inflated_dem',
                          channel_mask='channel_mask',channel_id='channel_id', 
                          out_name='hand',dirmap=dirmap,nodata_in_dem=fill_value)
        ...
        grid.compute_hillslope(fdir='dir', channel_mask='channel_mask', bank_mask='bank_mask')        
        self.hillslope = np.asarray(grid.view('hillslope'))

        # create hillslope masks
        hillmask = []
        for k1 in range(1,4):
            hillmask.append(np.where(np.logical_or(self.hillslope==4,self.hillslope==k1),self.drainage_id,0))
        ...
        if useMultiprocessing:
            ...
            aspect2d_catchment_mean = set_aspect_to_hillslope_mean_parallel(self.drainage_id,self.aspect,self.hillslope,npools=npools)
            if verbose:
                etime = time.time()
                print('\nTime to complete set_aspect_to_hillslope_mean_parallel: {:.3f} seconds'.format(etime-stime))
                    
        else:
            # serial version
            if verbose:
                stime = time.time()
            aspect2d_catchment_mean = set_aspect_to_hillslope_mean_serial(self.drainage_id,self.aspect,self.hillslope)
            if verbose:
                etime = time.time()
                print('\nTime to complete set_aspect_to_hillslope_mean_serial: {:.3f} seconds'.format(etime-stime))
